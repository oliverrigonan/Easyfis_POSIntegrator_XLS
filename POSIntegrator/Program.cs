    ...
    class Program
    {
        ...
        public static void SyncStockInAndStockOut(String xlsINPath, String xlsOTPath, String database)
        {
            try
            {
                ...
                // IN Files 
                // ========
                List<String> INfiles = new List<String>(Directory.EnumerateFiles(xlsINPath));
                if (INfiles.Any())
                {
                ...
                // OT Files 
                // ========
                List<String> OTfiles = new List<String>(Directory.EnumerateFiles(xlsOTPath));
                if (OTfiles.Any())
                {
                    foreach (var file in OTfiles)
                    {
                        Excel.Application xlApp = new Excel.Application();
                        Excel.Workbook xlWorkBook = xlApp.Workbooks.Open(file, 0, true, 5, "", "", true, Microsoft.Office.Interop.Excel.XlPlatform.xlWindows, "\t", false, false, 0, true, 1, 0);
                        Excel.Worksheet xlWorkSheet = (Excel.Worksheet)xlWorkBook.Worksheets.get_Item(1);
        ...
        {
            String xlsINPath = "", xlsOTPath = "", database = "";
                ...
                try
                {
                    if (!xlsINPath.Equals(""))
                    {
                        if (!xlsOTPath.Equals(""))
                        {
                            ...
                            if (INfiles.Any())
                            {
                                SyncStockInAndStockOut(xlsINPath, xlsOTPath, database);
                            }
                            else
                            {
                                Console.WriteLine("No IN XLS Found...");
                            }
                            ...
                            {
                                Console.WriteLine("No OT XLS Found...");
                            }
                        }
                        else
                        {
                            Console.WriteLine("Please provide OT XLS path.");
                        }
                    }
                    else
                    {
                        Console.WriteLine();
                        Console.WriteLine("Please provide IN XLS path.");
                    }
                }
                catch (Exception e)
                {
